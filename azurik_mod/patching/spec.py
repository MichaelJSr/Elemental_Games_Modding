"""PatchSpec and ParametricPatch — descriptors for every patch site.

A patch pack holds a mixed list of these two shapes:

- `PatchSpec`:       fixed byte swap at a VA.  Used by FPS / QoL packs.
- `ParametricPatch`: float-valued rewrite at a VA whose bytes are derived
                    from a user parameter at apply time.  Used for
                    sliders (gravity, player speed, etc.).  A VA of 0
                    + size of 0 marks a "virtual" slider whose value is
                    consumed by a custom apply_* function instead of a
                    direct XBE rewrite (e.g. walk_speed_scale which
                    writes into characters.xbr).
"""
