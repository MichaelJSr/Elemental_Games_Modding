...
class ScaffoldPlan:
    ...
    # ``--emit-test`` adds a pytest module beside ``__init__.py``
    # that asserts the feature registered + that trampoline
    # constants match what the scaffolder decided on.  Tests
    # using #19 land here as soon as ``write_scaffold`` runs.
    test_path: Path | None = None
    test_body: str | None = None

    @property
    def files(self) -> list[Path]:
        base = [self.init_py_path, self.shim_c_path, self.readme_path]
        if self.test_path is not None:
            base.append(self.test_path)
        return base
...
def plan_scaffold(name: str, *,
    ...
    test_body: str | None = None
    if emit_test:
        test_path = feature_dir / f"test_{name}.py"
        test_body = _render_test_module(
            name=name,
            hook_va=hook_va,
...
def write_scaffold(plan: ScaffoldPlan) -> None:
    ...
    if plan.test_path is not None and plan.test_body is not None:
        plan.test_path.write_text(plan.test_body, encoding="utf-8")


# ---------------------------------------------------------------------------
# Emit-test template renderer (#19)
# ---------------------------------------------------------------------------


def _render_test_module(*, name: str,
                        hook_va: int | None,
                        replaced_bytes: bytes | None,
                        category: str) -> str:
    """Generate a pytest module that pins the scaffolded shim's
    constants.

    The goal is to detect drift: if someone renames the feature,
    moves its hook VA, or changes the bytes the trampoline
    replaces, the test fails loudly and the diff reviewer sees
    exactly what moved.  The test deliberately stays small
    (<60 LOC) so authors can read + extend it without
    ceremony.
    """
    hook_hex = (f"0x{hook_va:08X}" if hook_va is not None
                else "None")
    replaced_literal = (
        "bytes.fromhex(" + repr(replaced_bytes.hex()) + ")"
        if replaced_bytes else "None")
    return f'''"""Regression tests for the ``{name}`` shim.

Auto-generated by ``azurik-mod new-shim --emit-test``.  Edit the
asserts below once the feature's behaviour solidifies — these are
starter guards, not a full test plan.

Run with::

    pytest azurik_mod/patches/{name}/test_{name}.py -vv
"""
...
class {_CamelCase(name)}FeatureRegistration(unittest.TestCase):
    """Pin the basic feature metadata so renames + category
    drift surface as test failures before any ISO is built."""

    def test_feature_is_registered(self) -> None:
        feature = get_pack({name!r})
        self.assertIsNotNone(
            feature,
            msg="feature failed to register at import time")

    def test_feature_category(self) -> None:
        feature = get_pack({name!r})
        self.assertEqual(feature.category, {category!r})


class {_CamelCase(name)}TrampolineConstants(unittest.TestCase):
    """Pin the scaffolder-picked trampoline constants.

    If you move the hook or grow/shrink ``replaced_bytes``, update
    the expected values below — the aim is to FORCE that
    conscious decision, not to silently drift.
    """

    EXPECTED_HOOK_VA = {hook_hex}
    EXPECTED_REPLACED = {replaced_literal}

    def test_scaffold_constants_stable(self) -> None:
        feature = get_pack({name!r})
        sites = list(feature.sites)
        self.assertTrue(sites, msg="feature has no patch sites")
        if self.EXPECTED_HOOK_VA is not None:
            self.assertIn(
                self.EXPECTED_HOOK_VA,
                [s.va for s in sites],
                msg="expected hook VA missing from patch sites")
        if self.EXPECTED_REPLACED is not None:
            matching = [s for s in sites
                        if s.va == self.EXPECTED_HOOK_VA]
            self.assertTrue(matching)
            self.assertEqual(
                matching[0].original,
                self.EXPECTED_REPLACED,
                msg="replaced_bytes drift — re-run new-shim "
...
def _CamelCase(name: str) -> str:
    """Convert ``snake_case`` → ``SnakeCase`` (no mixed acronyms,
    keeps it simple)."""
