...
The Patches page renders 10 working `ParametricSlider` widgets under `player_physics`: `gravity`, `walk_speed_scale`, `swim_speed_scale`, `jump_speed_scale`, `air_control_scale`, `flap_height_scale` (1st flap), `flap_below_peak_scale` (2nd+ flaps when >6m below peak), `wing_flap_ceiling_scale` (raises the wing-flap altitude ceiling by shim-scaling the peak_z latch at jump-init), `flap_entry_fuel_cost_scale` (scales the 1.0f per-flap entry drain at VA 0x8934E — 0.1 ≈ 10× more flaps, 0.0 = infinite, negative = refund), and `flap_descent_fuel_cost_scale` (scales the 100.0 descent-penalty drain at VA 0x893CE; pair with `wing_flap_ceiling_scale` so descent flaps actually stay usable).  Slider values live on `AppState.pack_params["player_physics"]` and carry a long-form description rendered via a hover-tooltip `ⓘ` glyph next to the label; sliders accept out-of-range values via the text box for expert tuning.

An additional **experimental** pack `animation_root_motion_scale` renders its own slider that globally scales every animation-driven player translation by hooking the vtable-commit call at the end of `anim_apply_translation` (VA 0x43066).  See `docs/LEARNINGS.md` § "Animation root-motion vtable-commit hook" for the rationale.

**Round 10 purge, selectively reversed**: rounds 7–8 attempted to revive `flap_at_peak`, `root_motion_roll`, `root_motion_climb`, and `slope_slide_speed` as hand-assembled shims; round-10 deleted them after user testing reported no effect, but the round-11.6 forensic traced that symptom to a GUI wiring bug (slider values never reached the pack).  Round-11.8 restored all four packs with the slider values now correctly routed through `pack_params_json`.  `root_motion_roll` and `root_motion_climb` are marked `deprecated=True` (hidden from the GUI) in favour of the central `animation_root_motion_scale` hook; they remain importable for CLI / RE use.  `no_fall_damage`, `infinite_fuel`, and `wing_flap_count` remain deleted in favour of config-editor workarounds.
