...
from patches.xbe_utils import PatchSpec, apply_patch_spec
...
FPS_PATCH_SITES: list[PatchSpec] = [
    ...
    # threshold.
    #
    # CAP IS KEPT AT 2 (matching the original game's hardcoded limit).
    # safety_critical=True: the shipped game never drives the sim loop
    # past 2 calls per frame, so that is the only reentrancy depth we
    # have evidence the engine tolerates.  NOTE: the on-death BSOD that
    # prompted the original 4->2 change also reproduces on unpatched
    # 30fps Azurik, meaning the cap was not its cause — it is a pre-
    # existing engine bug.  Regardless, raising the cap would exceed
    # vanilla reentrancy and is therefore never the correct direction;
    # any real fix would require code injection to make the sim
    # properly re-entrant.
    PatchSpec(
        label="60 FPS FISTP truncation + step clamp (anti-death-spiral)",
        va=0x059AFD,
        original=bytes([
        ...
        patch=bytes([
            ...
            0x89, 0x74, 0x24, 0x14,                            # MOV [ESP+0x14], ESI
            0x83, 0xFE, 0x02,                                  # CMP ESI, 0x2  <-- BSOD guard
    ...
    # Patch 5: catchup code — cap to 2 steps, keep remainder (VA 0x59B37).
    #
    ...
    # block:
    #   1. Caps ESI to the max step count (2)
    ...
    #
    # safety_critical=True: the cap of 2 must match Patch 4's CMP ESI, 2
    # so the TRUNC pre-check and the catchup post-check agree.  See the
    # Patch 4 header for the rationale — the cap matches vanilla
    # reentrancy; the on-death BSOD that motivated keeping it at 2 also
    # reproduces on vanilla 30fps and is a pre-existing engine bug.
    # ESI is loaded via PUSH 0x2 / POP ESI (byte-equivalent substitute
    # for MOV ESI, 2) so the 30-byte block length is preserved.  A
    # single FADD ST0,ST0 yields 2*dt; the freed 2 bytes from the
    # removed second FADD become NOP NOP.
    PatchSpec(
        label="60 FPS catchup (ESI=2, remainder=raw_delta-2*dt)",
        va=0x059B37,
        original=bytes([
