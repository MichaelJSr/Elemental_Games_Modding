...
# ---------------------------------------------------------------------------
# Obsidian fist pump animation skip
# ---------------------------------------------------------------------------
# Patch 6 bytes at file offset 0x0489C3 to skip the fist pump on first obsidian.
...
# ---------------------------------------------------------------------------
# Per-pickup fist pump animation skip
# ---------------------------------------------------------------------------
# Player state machine at 0x2B0F2: state 0x1E checks absorbed entity type
# (+0x148); if non-zero, plays animation 0x52.  Patch JE to JMP to always skip.
...
def apply_qol_patches(xbe_data: bytearray, args) -> None:
    ...
    Respects per-patch opt-out flags on args:
      ...
      --no-obsidian-anim → skip obsidian animation patch
      --no-fist-pump     → skip fist pump animation patch
    """
    ...
    if not getattr(args, 'no_obsidian_anim', False):
        apply_xbe_patch(xbe_data, "Disabled obsidian first-pickup notification",
                        OBSIDIAN_ANIM_OFFSET, OBSIDIAN_ANIM_ORIGINAL, OBSIDIAN_ANIM_PATCH)
    else:
        print(f"  Obsidian animation patch — skipped")

    if not getattr(args, 'no_fist_pump', False):
        apply_xbe_patch(xbe_data, "Disabled per-pickup fist pump animation",
                        FIST_PUMP_OFFSET, FIST_PUMP_ORIGINAL, FIST_PUMP_PATCH)
    else:
        print(f"  Fist pump patch — skipped")
